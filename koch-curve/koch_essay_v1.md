...
§3). For the polygons $P_n$ the parameter is a genuine arc-length fraction,
since all $4^n$ segments have the same Euclidean length $3^{-n}$; in the
limit, where arc length is infinite, the same parameter instead represents
the *natural mass distribution* of the fractal — the self-similar measure
with equal weights $1/4$ on the four quarters, which (the ratios being
equal) is a constant multiple of the $s$-dimensional Hausdorff measure
restricted to $K$, $s=\log 4/\log 3$. Figure 1 shows $P_n$ for increasing
$n$.
...
consecutive quarters. (The bar in $S_2$ is not a reflection:
$\bar\omega=e^{-i\pi/3}$ is just a constant, so $S_2$ is a rotation by
$-\pi/3$ with ratio $1/3$, an orientation-*preserving* similitude like the
other three.) The Koch curve $K$ is the (unique) non-empty compact set
...
chord $P_0(t)=t$, $\|\varphi-P_0\|_\infty=\max_t|\varphi(t)-t|=\sqrt3/6=E_0$,
attained at the apex $t=\tfrac12$ of the level-1 triangle,
$\varphi(\tfrac12)=\tfrac12+\tfrac{i\sqrt3}{6}$. Hence
$\|\varphi-P_n\|_\infty=3^{-n}E_0$ exactly. $\blacksquare$

The script verifies the exact rate (the column $E_n\cdot3^n$ is constant):

```
E_0 = max_t |phi(t) - t| = 0.288675   (sqrt(3)/6 = 0.288675)
  n     E_n          E_n * 3^n      (-> E_0)
...
*Proof.*
**Upper bound.** Let $t\ne s$ and choose $n$ so that
$4^{-(n+1)}<|t-s|\le4^{-n}$. Use the exact error
$\|\varphi-P_n\|_\infty=3^{-n}E_0$ (Theorem 1) together with the geometry of
$P_n$: each of its $4^n$ segments has parameter length $4^{-n}$ and Euclidean
length $3^{-n}$, and is a straight line. Hence
$$
|\varphi(t)-\varphi(s)|
\;\le\;\underbrace{2\,3^{-n}E_0}_{\text{error vs. }P_n}
\;+\;\underbrace{|P_n(t)-P_n(s)|}_{\le\,2\,3^{-n}}
\;\le\;2(1+E_0)\,3^{-n},
$$
where $|P_n(t)-P_n(s)|\le2\,3^{-n}$: because $|t-s|\le4^{-n}$, the parameters
$t$ and $s$ lie in at most two *adjacent* level-$n$ segments, so their
distance *along* the polygon is at most the sum of the two full segment
lengths, $2\cdot3^{-n}$, and the Euclidean distance is bounded by the path
distance. Since
$|t-s|>4^{-(n+1)}$ gives
$3^{-n}=4^{-n\alpha}=4^{\alpha}\,4^{-(n+1)\alpha}<4^{\alpha}|t-s|^{\alpha}$, we
obtain
$$
|\varphi(t)-\varphi(s)|\;\le\;2(1+E_0)\,4^{\alpha}\,|t-s|^{\alpha},
$$
so $\varphi\in C^{0,\alpha}$ (with $C=2(1+E_0)4^\alpha=6(1+E_0)$, using
$4^\alpha=3$).
...
critical value  V_{p*} on vertex partitions (exact, all levels):
   ...
   multi-scale (levels 1..8, 65537 pts):  V_{p*} = 1.0000000000
   (supremum is exactly 1: a C^{0,alpha} curve has finite p-variation for
    all p >= 1/alpha = p*, and the critical value is attained by the
    vertex partitions, where 4*3^{-p*}=1 makes each level contribute 1)
```
...
- **Symmetry.** The curve is symmetric about the vertical line $x=\tfrac12$:
  ...
  forces every $c_k$ ($k\ge1$) to be *purely imaginary* (the real part
  vanishes), so $c_1=0.083968\,i$. Indeed, with $t=1-u$ and
  $e^{-2\pi i k}=1$,
  $c_k=\int_0^1\varphi(1-u)\,e^{-2\pi i k(1-u)}\,du
  =\int_0^1(1-\overline{\varphi(u)})\,e^{2\pi i k u}\,du
  =-\overline{c_k}$,
  since $\int_0^1 e^{2\pi i k u}\,du=0$ for $k\ge1$; hence
...
- **Not absolutely convergent.** The envelope is $O(k^{-1})$ (fit slope
  ...
  the coefficients.) Note the contrast with the Weierstrass function of the
  introduction: the Weierstrass series $\sum_n a^n\cos(b^n\pi x)$ is
  *absolutely* convergent, since $\sum_n a^n<\infty$, whereas the Koch
  parametrization does not belong to the **Wiener algebra**
  $A(\mathbb T)=\{f:\sum_k|c_k|<\infty\}$ — its summable lacunary skeleton
  is not enough, because the non-lacunary coefficients filling the gaps
  between the $4^n$ force the total sum to diverge.
...
| $p$-variation | finite iff $p\ge\log 4/\log 3$ (critical value $=1$, attained by vertex partitions) | §6.3 |
...
8. **T. J. Lyons, M. Caruana, T. Lévy**, *Differential Equations Driven by
   Rough Paths*, in *École d'été de probabilités de Saint-Flour XXXIV—2004*
   (ed. J. Picard), Lecture Notes in Mathematics **1908**, Springer, 2007,
   1–119. (The $p$-variation / rough-paths viewpoint, §6.3.)
